...
def serialize_post_optimized(post):
    return {
        ...
        'slug': post.slug,
        'tags': [serialize_tag(tag) for tag in post.tags.prefetch_related("posts")], #.prefetch_related('posts')],
...
def index(request):
    most_popular_posts = Post.objects.prefetch_related('author').annotate(num_likes=Count('likes')).order_by('-num_likes')[:5]
    most_fresh_posts = Post.objects.prefetch_related('author').order_by('published_at')[:5]


    most_popular_tags = Tag.objects.prefetch_related('posts').annotate(num_posts=Count('posts')).order_by('-num_posts')[:5]
...
def post_detail(request, slug):
    ...
    most_popular_tags = Tag.objects.all()[:5]
